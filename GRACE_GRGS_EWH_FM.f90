...
!change in surface mass density from changes in the spherical harmonic geoid coefficients [Wahr et al., 1998] eq. 14

! EWT versus Forwadrd model 

!a time avarage of 293 solutions is removed to each 10-days solution

PROGRAM GRACE_GRGS_deltasigma
...
double precision :: sum_s,b_wahr,cons_wahr,vcos


INTEGER :: i,j,l_s,m_s,sum_i,il,k

integer, dimension(:,:), allocatable :: degree,order

double precision, dimension(:,:), allocatable :: cnm,snm,delta_sigma,pleg

double precision, dimension(:), allocatable :: landa,phi

integer, dimension(:), allocatable :: n_lov,m,n,ma,na

double precision, dimension(:), allocatable :: wahr,c_lov,s_lov,k_lov,c,s,ca,sa
...
!   allocate arrays
    allocate (cnm(51,51),snm(51,51),degree(51,51),order(51,51))
    allocate (c(1326),s(1326),n(1326),m(1326),ca(1326),sa(1326),na(1326),ma(1326))
    allocate (chara(1326))
...
!making a grid of one degree by one degree for whole world

allocate (phi(181),landa(360))
...
read (k+300, 101) chara(i),n(i),m(i),c(i),s(i)
...
OPEN(k+600,file=namesw(k),form='formatted',status='old')
...
write (k+600, 119) landa(i)/deg2rad,phi(j)/deg2rad,delta_sigma(i,j)
...
end Program GRACE_GRGS_deltasigma
